import React from "react";
import style from "./WinSkillsBox.module.css";


function WinSkillsBox(props) {
  return (
    <div className={style.Boxcontainer}>
      <div className={style.boxHead}>
        <h5>{props.title}</h5>
        <div className={style.listBox}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            aria-hidden="true"
            role="img"
            className="iconify iconify--custom"
            width="1em"
            height="1em"
            viewBox="0 0 16 16"
            style={{ touchAction: "none" }}
          >
            <g fill="currentColor">
              <path d="M2.67 8c0-.37.3-.67.66-.67h9.34a.67.67 0 1 1 0 1.34H3.33A.67.67 0 0 1 2.67 8" />
            </g>
          </svg>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            aria-hidden="true"
            role="img"
            className="iconify iconify--custom"
            width="1em"
            height="1em"
            viewBox="0 0 24 24"
            style={{ touchAction: "none" }}
          >
            <g fill="currentColor">
              <path d="M8.2 4c-1.12 0-1.82-.05-2.56.33a3 3 0 0 0-1.31 1.3C3.95 6.39 4 7.09 4 8.2v7.6c0 1.12-.05 1.82.33 2.56a3 3 0 0 0 1.3 1.31c.75.38 1.45.33 2.57.33h7.6c1.12 0 1.82.05 2.56-.33a3 3 0 0 0 1.31-1.3c.38-.75.33-1.45.33-2.57V8.2c0-1.12.05-1.82-.33-2.56a3 3 0 0 0-1.3-1.31C17.61 3.95 16.91 4 15.8 4Zm0 2h7.6c1.12 0 1.54.05 1.65.1a1 1 0 0 1 .44.45c.06.11.11.53.11 1.65v7.6c0 1.12-.05 1.54-.1 1.65a1 1 0 0 1-.45.44c-.11.06-.53.11-1.65.11H8.2a6 6 0 0 1-1.65-.1 1 1 0 0 1-.44-.45A6 6 0 0 1 6 15.8V8.2c0-1.12.05-1.54.1-1.65a1 1 0 0 1 .45-.44C6.66 6.05 7.08 6 8.2 6" />
            </g>
          </svg>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            aria-hidden="true"
            role="img"
            className="iconify iconify--custom"
            width="1em"
            height="1em"
            viewBox="0 0 16 16"
            style={{ touchAction: "none" }}
          >
            <g fill="currentColor">
              <path d="M3.53 3.53a.67.67 0 0 1 .94 0L8 7.06l3.53-3.53a.67.67 0 1 1 .94.94L8.94 8l3.53 3.53a.67.67 0 1 1-.94.94L8 8.94l-3.53 3.53a.67.67 0 1 1-.94-.94L7.06 8 3.53 4.47a.67.67 0 0 1 0-.94" />
            </g>
          </svg>
        </div>
      </div>

      <ul className={style.skillboxBody} aria-label="List of my key skills">
       { props.skills.map((skill,index)=>{
        return(
          <li className={style.individaulskillbox} key={index}>
            <img  className={style.skillsicon} src={skill.image} alt={skill.alt} height={"75px"} width={"150px"}></img>
          </li>
        );
       })}
        
      </ul>
    </div>
  );
}

export default WinSkillsBox;
